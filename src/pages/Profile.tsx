  ...
  const fetchUserStats = async () => {
    ...
    try {
      ...
      const { data: ratingsData } = await supabase
        .from('ratings')
        .select('rating')
        .in('resource_id', (await supabase.from('resources').select('id').eq('uploaded_by', user.id)).data?.map(r => r.id) || []);
      const averageRating = ratingsData && ratingsData.length > 0
        ? (ratingsData.reduce((sum, r) => sum + (r.rating || 0), 0) / ratingsData.length).toFixed(1)
        : 0;
      setUserStats({
        resourcesUploaded: uploadsCount || 0,
        totalLikes,
        totalDownloads,
        averageRating
      });
